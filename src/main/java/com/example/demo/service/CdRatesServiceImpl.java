...
import com.example.demo.dao.CdArchiveRatesRepository;
import com.example.demo.dao.CdRatesRepository;
...
public class CdRatesServiceImpl implements CdRatesService {
	...
	public List<CdRatesDTO> getData(int zipCode) {
		...
		String state = getState(String.valueOf(zipCode));
		List<CdRates> cdRatesResponse = (List<CdRates>) cdRatesRepository.findByStateName(state);
		List<CdRatesDTO> cdRatesList = new ArrayList<>();
		for (CdRates cdRates : cdRatesResponse) {
			CdRatesDTO cdRatesDTO = new CdRatesDTO();
			cdRatesDTO.setIntrstRate(cdRates.getIntrstRate());
			cdRatesDTO.setTermLength(cdRates.getTermLength());
			cdRatesDTO.setMinDepositOpen(cdRates.getMindo());
			cdRatesDTO.setMaxDepositPermit(cdRates.getMaxdp());
	...
	public List<CdManagerRatesDTO> getManagerData(int zipCode) {
		...
		for (CdRates cdRates : cdRatesResponse) {
			...
			cdManagerRatesDTO.setMinDepositOpen(cdRates.getMindo());
			cdManagerRatesDTO.setMaxDepositPermit(cdRates.getMaxdp());
	...
	@Override
	public List<CdHistoryRatesDTO> getHistoryData(int zipCode) {
 
		String state = getState(String.valueOf(zipCode));
		List<ArchiveCdRates> cdHisRatesResponse = (List<ArchiveCdRates>) cdArchiveRatesRepository
				.findByStateName(state);
		System.out.println("GetCdHistoryData from DB");
		...
		for (ArchiveCdRates cdHisRates : cdHisRatesResponse) {
			...
			cdHisRatesDTO.setHistoryIntrstRate(cdHisRates.getIntrstRate());
	...
	public List<CdManagerHistoryRatesDTO> getManagerHistoryData(int zipCode) {
		...
		List<ArchiveCdRates> cdHisRatesResponse = (List<ArchiveCdRates>) cdArchiveRatesRepository
				.findByStateName(state);
		System.out.println("Get ManagerCdHistoryData from DB");
